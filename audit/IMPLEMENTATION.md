...
| # | Action | Refs | Complexity | Status | Owner | Notes |
|---|---|---|:-:|---|---|---|
| P1-1 | Flip every body-address money route to `requireAuth` (SIWE) | [BE-003](backend.md), [BE-004](backend.md), [BE-005](backend.md), [BE-009](backend.md), [SEC-003](security.md) | 2 | **in review** | claude | Done in PR #38: withdraw, player/profile, tournament cancel/reclaim, cosmetics gift, cosmetics grant, deposit/notify. Done in follow-up batch: poker chip purchase, poker chip cashout, instant lottery play, cosmetics admin create-item / item PATCH / bulk-shop-listed / tier-pricing / item-owners, follow, unfollow. **Still todo**: blackjack-multi `tipDealer` (lives in WebSocket handler, blocked on WS cookie auth work). |
