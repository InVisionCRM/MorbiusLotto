-- Migration: remove on-chain settlement remnants
-- Step 1: Recreate get_global_analytics without depending on settlements table.
-- Note: Dropping the settlements table can block if other connections are still
-- querying it (e.g. analytics polling). Do that in a follow-up migration once
-- the updated function is deployed and the server is restarted.

-- Fail fast if the function is locked by in-flight queries.
SET lock_timeout = '3s';
