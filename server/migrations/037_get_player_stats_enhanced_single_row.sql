-- Migration: Ensure get_player_stats_enhanced never returns "more than one row" from a subquery
-- All scalar subqueries use LIMIT 1; wallet lookup uses LOWER() to match 034.
...
BEGIN
    ...
    players_with_higher_total AS (
        ...
        GROUP BY p2.id
        HAVING COALESCE(SUM(g2.total_bet_amount), 0) > (SELECT player_total_bet FROM player_total LIMIT 1)
    ),
    player_rank AS (
