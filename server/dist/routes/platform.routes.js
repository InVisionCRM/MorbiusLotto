...
function registerPlatformRoutes({ app, dbService, chainAnalytics, wsService, instantLotteryService, getAnalyticsCacheKey, getCachedAnalytics, setCachedAnalytics, }) {
    ...
    const instantLotteryErrorBySelector = {
    ...
    const resolveInstantLotteryPlayError = (error) => {
        const msg = error instanceof Error ? error.message : String(error ?? 'Unknown error');
        if (msg.includes('Valid wallet')
        ...
        for (const [selector, mapped] of Object.entries(instantLotteryErrorBySelector)) {
            if (inspect.includes(selector.toLowerCase()))
                return mapped;
        }
    ...
    app.get('/api/analytics/platform', async (_req, res) => {
        ...
        if (cached != null) {
            return (0, json_1.sendJson)(res, cached);
        }
        try {
    ...
    app.get('/api/analytics/live-presence', (_req, res) => {
        try {
            (0, json_1.sendJson)(res, wsService.getLivePresenceByGame());
        }
        catch (error) {
    ...
    app.get('/api/analytics/global-metrics', async (req, res) => {
        ...
        try {
            ...
            const totalWagered = range === 'all' ? blackjackWagered + plinkoWagered + kenoWagered + lotteryWagered + bigWheelWagered : blackjackWagered;
