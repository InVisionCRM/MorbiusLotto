...
exports.registerMoneyRoutes = registerMoneyRoutes;
const logger_1 = require("../utils/logger");
const json_1 = require("../http/json");
function statusForMoneyError(message) {
    ...
    if (message.includes('No credited deposit found'))
        return 404;
    if (message.includes('temporarily unavailable') || message.includes('temporarily limited') || message.includes('Could not load transaction'))
        return 503;
    if (message.includes('Insufficient balance'))
        return 400;
    if (message.includes('not found or not yet mined') || message.includes('reverted on-chain') || message.includes('Could not verify deposit amount') || message.includes('not sent to the Blackjack contract'))
        return 400;
    return 500;
}
function registerMoneyRoutes({ app, moneyService }) {
    ...
    app.get('/api/admin/pending-transfers', async (req, res) => {
        try {
            ...
            (0, json_1.sendJson)(res, { type, rows, limit, offset, hasMore: rows.length === limit });
        }
        catch (error) {
