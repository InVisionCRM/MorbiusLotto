...
function computeBuyInChips(bigBlindChips) {
 ...
 */
async function ensureBotChipBalances(addresses, minChips) {
    if (POKER_BOT_SKIP_DB) {
        console.log('[Bot] POKER_BOT_SKIP_DB is set — skipping DB provisioning. Confirm player_poker_chips + players yourself.');
        return;
    }
    if (!DATABASE_URL) {
        throw new Error('DATABASE_URL is required so bots get poker chip balances. ' +
    ...
    try {
        for (const addr of addresses) {
         ...
         VALUES ($1, '0')
         ON CONFLICT (wallet_address) DO UPDATE SET last_seen = NOW()`, [normalized]);
            await pool.query(`INSERT INTO player_poker_chips (wallet_address, balance)
         VALUES ($1, $2::NUMERIC)
         ON CONFLICT (wallet_address)
         DO UPDATE SET balance = GREATEST(player_poker_chips.balance, EXCLUDED.balance), updated_at = NOW()`, [normalized, minChips.toString()]);
...
async function getTournamentBuyInChips(tournamentId) {
    if (BOT_ADDRESSES.length === 0)
...
async function runBot(address, tableId, buyInChips) {
    const tag = `[Bot ${address.slice(0, 8)}]`;
...
async function main() {
    ...
    if (tournamentId) {
        ...
        const minChipFloor = buyInChips > 0n ? buyInChips * 20n : 1000000n;
    ...
    const promises = botAddrs.map((addr) => runBot(addr, tableId, buyInChips).catch((err) => {
        console.error(`[Bot ${addr.slice(0, 8)}] Fatal: ${formatError(err)}`);
    }));
    await Promise.all(promises);
    console.log('\n=== All bots finished ===');
