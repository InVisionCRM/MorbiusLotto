...
const pf = new provably_fair_service_1.ProvablyFairService();
...
function cardIndexToRank(idx) {
    return (idx % 13) + 1;
}
/** Dealer upcard "value" for basic strategy: 2-10, 11 for Ace */
function dealerUpcardValue(cardIdx) {
 ...
 */
function shouldHit(total, isSoft, dealerUpcardVal) {
    if (total >= 17)
        return false;
...
function runOneHand(deck, position) {
    const wagered = 1;
    ...
    while (true) {
        ...
        if (d.total >= 17 && !(d.total === 17 && d.hasAce))
            break;
    ...
    if (finalDealerTotal > 21)
        return { payout: 2, wagered };
    if (playerTotalNow > finalDealerTotal)
        return { payout: 2, wagered };
    if (playerTotalNow < finalDealerTotal)
        return { payout: 0, wagered };
    return { payout: 1, wagered };
...
main();
//# sourceMappingURL=simulate-blackjack-house-edge.js.map