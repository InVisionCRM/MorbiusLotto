...
async function expireTurn(handId) {
    await setup_1.testPool.query(`UPDATE poker_hands SET turn_started_at = NOW() - INTERVAL '120 seconds' WHERE id = $1`, [handId]);
}
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe('Poker Auto-Fold / Auto-Check', () => {
    describe('auto-check when no outstanding bet', () => {
        it('auto-checks the BB when limped to in heads-up', async () => {
            ...
            const actionResult = await setup_1.testPool.query(`SELECT action FROM poker_hand_actions
         WHERE hand_id = $1 AND player_address = $2 AND action NOT IN ('blind')
         ORDER BY "order" DESC LIMIT 1`, [handId, bbAddr]);
    ...
    describe('auto-fold when facing a bet', () => {
        it('auto-folds when facing an unmatched raise', async () => {
            ...
            const actionResult = await setup_1.testPool.query(`SELECT action FROM poker_hand_actions
         WHERE hand_id = $1 AND player_address = $2 AND action NOT IN ('blind')
         ORDER BY "order" DESC LIMIT 1`, [handId, timedOutAddr]);
    ...
    describe('consecutive timeout tracking', () => {
        it('increments consecutive_timeouts on each auto-fold', async () => {
            ...
            const beforeResult = await setup_1.testPool.query('SELECT consecutive_timeouts FROM poker_seats WHERE table_id = $1 AND player_address = $2', [tableId, actingAddr]);
            ...
            const afterResult = await setup_1.testPool.query('SELECT consecutive_timeouts FROM poker_seats WHERE table_id = $1 AND player_address = $2', [tableId, actingAddr]);
        ...
        it('resets consecutive_timeouts on voluntary action', async () => {
            ...
            await setup_1.testPool.query(`UPDATE poker_seats SET consecutive_timeouts = 3
         WHERE table_id = $1 AND player_address = $2`, [tableId, PLAYER_1]);
            const handState = await pokerGameService.startHand(tableId);
            const handId = handState.currentHand.handId;
            let state = await pokerGameService.getTableState(tableId, null);
            const acting = state.currentHand.actingPosition;
            ...
            if (actingAddr === PLAYER_1) {
                ...
                const result = await setup_1.testPool.query('SELECT consecutive_timeouts FROM poker_seats WHERE table_id = $1 AND player_address = $2', [tableId, PLAYER_1]);
    ...
    describe('canCheck logic edge cases', () => {
        it('auto-checks on the flop when first to act (no bets)', async () => {
           ...
           ORDER BY "order" DESC LIMIT 1`, [handId, actingAddr]);
                expect(actionResult.rows[0]?.action).toBe('check');
            }
        });
    });
...
//# sourceMappingURL=poker-autofold.test.js.map