...
const poker_chip_wallet_1 = require("./services/poker-chip-wallet");
...
let _bjTotalsCache = { deposited: '0', withdrawn: '0' };
let _bjTotalsRefreshing = false;
function refreshBjTotalsBackground(chainAnalytics) {
    if (_bjTotalsRefreshing)
        return;
...
async function initializeServices() {
    try {
        ...
        merkleDropsService = new merkle_drops_service_1.MerkleDropsService(dbService.getPool());
        ...
        app.post('/api/player/profile', express_1.default.json(), async (req, res) => {
            try {
                ...
                const profileDisplayMode = rawProfileDisplayMode === 'photo' || rawProfileDisplayMode === 'avatar' ? rawProfileDisplayMode : undefined;
        ...
        const instantLotteryPlayLimiter = (0, express_rate_limit_1.default)({
            windowMs: 1 * 60 * 1000,
            max: 30,
        ...
        app.post('/api/lottery/instant/play', instantLotteryPlayLimiter, async (req, res) => {
            try {
                ...
                logger_1.logger.error('Instant lottery play error:', error);
        ...
        app.get('/api/poker/chips/balance', async (req, res) => {
            try {
                ...
                const balance = await (0, poker_chip_wallet_1.getPokerChipBalance)(dbService.getPool(), normalized);
                return res.status(200).json({ balance: balance.toString() });
            }
            catch (error) {
                logger_1.logger.error('Error fetching poker chip balance:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        app.post('/api/poker/chips/purchase', express_1.default.json(), async (req, res) => {
            try {
                ...
                if (chipsBn <= 0n)
                    return res.status(400).json({ error: 'chips must be positive' });
                if (chipsBn > BigInt(Number.MAX_SAFE_INTEGER)) {
                    return res.status(400).json({ error: 'chips amount too large' });
                }
                const wei = (0, poker_chip_scale_1.chipsToWei)(Number(chipsBn));
             ...
             RETURNING balance`, [normalized, wei.toString()]);
                    ...
                    await (0, poker_chip_wallet_1.applyPokerChipDelta)(client, normalized, chipsBn, 'purchase');
                });
                const newBal = await (0, poker_chip_wallet_1.getPokerChipBalance)(dbService.getPool(), normalized);
                ...
                logger_1.logger.error('Poker chip purchase error:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        app.post('/api/poker/chips/cashout', express_1.default.json(), async (req, res) => {
            try {
                const { address, chips } = req.body ?? {};
                if (!address || typeof address !== 'string' || !/^0x[a-fA-F0-9]{40}$/i.test(address)) {
                    return res.status(400).json({ error: 'address required' });
                ...
                const wei = (0, poker_chip_scale_1.chipsToWei)(Number(chipsBn));
                await dbService.withTransaction(async (client) => {
                    await (0, poker_chip_wallet_1.applyPokerChipDelta)(client, normalized, -chipsBn, 'cashout');
                    await client.query(`INSERT INTO players (wallet_address, balance) VALUES ($1, $2::NUMERIC)
             ON CONFLICT (wallet_address) DO UPDATE SET balance = players.balance + $2::NUMERIC, last_seen = NOW()`, [normalized, wei.toString()]);
                });
                const newBal = await (0, poker_chip_wallet_1.getPokerChipBalance)(dbService.getPool(), normalized);
                ...
                logger_1.logger.error('Poker chip cashout error:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        ...
        app.get('/api/tournament/completed', async (req, res) => {
            try {
                ...
                logger_1.logger.error('Error fetching completed tournaments:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        ...
        app.get('/api/tournament/:tournamentId/results', async (req, res) => {
            try {
                ...
                logger_1.logger.error('Error fetching tournament results:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        ...
        app.get('/api/tournament/:tournamentId/stats', async (req, res) => {
            try {
                ...
                logger_1.logger.error('Error fetching tournament stats:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        ...
        app.get('/api/tournament/:tournamentId/hands', async (req, res) => {
            try {
                ...
                const offset = Math.max(parseInt(req.query.offset) || 0, 0);
                const playerAddress = req.query.player || null;
                const hands = await tournamentService.getTournamentHands(tournamentId, limit, offset, playerAddress);
                sendJson(res, hands.map((h) => ({ ...h, potAmount: h.potAmount.toString() })));
            }
            catch (error) {
                logger_1.logger.error('Error fetching tournament hands:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        ...
        app.delete('/api/admin/poker/tables/:tableId', async (req, res) => {
            try {
                ...
                logger_1.logger.error('Error deleting poker table:', error);
                const msg = dbSchemaError(error);
                res.status(msg ? 503 : 500).json({ error: msg || 'Internal server error' });
            }
        });
        ...
        app.get('/api/admin/health', async (req, res) => {
            try {
                ...
                try {
                    const balance = await readMorbiusBalance(contracts_1.LOTTERY_INSTANT_ADDRESS);
                ...
                const contractAddresses = {
                    ...
                    lottery: contracts_1.LOTTERY_INSTANT_ADDRESS,
                };
                // Hot wallet (withdrawals): address + MORBIUS balance when configured
                let hotWalletAddress;
                let hotWalletMorbius;
                let hotWalletLowWarning;
        ...
        app.post('/api/deposit/notify', async (req, res) => {
            try {
                ...
                const hash = txHash;
                const blackjackAddr = (0, viem_1.getAddress)(contracts_1.BLACKJACK_ADDRESS);
                let receipt;
                try {
                ...
                for (const log of receipt.logs) {
                    if (log.address?.toLowerCase() !== contracts_1.BLACKJACK_ADDRESS.toLowerCase())
                        continue;
                    try {
        ...
        app.post('/api/admin/deposit/credit-shortfall', async (req, res) => {
            try {
                ...
                try {
                    ...
                    return res.status(400).json({ error: 'Invalid correctAmountWei' });
                }
                if (correctBigInt <= 0n)
                    return res.status(400).json({ error: 'correctAmountWei must be positive' });
                ...
                logger_1.logger.error('Error in admin deposit/credit-shortfall:', error);
                return res.status(500).json({ error: 'Internal server error' });
            }
        });
        // Shared RPC client (balance checks, legacy pending_withdrawal nonce verification, hot-wallet liquidity).
        const publicClient = (0, chain_client_1.getPublicClient)();
        const blackjackContractAddress = contracts_1.BLACKJACK_ADDRESS;
        ...
        try {
            ...
            if (orphaned.length > 0) {
                ...
                for (const row of orphaned) {
                    try {
                        ...
                        if (nonceUsed) {
                            ...
                            completedCount++;
                            logger_1.logger.warn('Startup: pending withdrawal was completed on-chain — marked completed (no refund)', {
                        ...
                        logger_1.logger.error('Startup: failed to check nonce on-chain — skipping (cron will retry)', {
                ...
                if (refundedCount > 0)
                    logger_1.logger.info(`Startup: expired ${refundedCount} orphaned pending withdrawal(s) and refunded balances`);
                if (completedCount > 0)
                    logger_1.logger.info(`Startup: marked ${completedCount} pending withdrawal(s) as completed (on-chain nonce used)`);
            }
        }
        catch (err) {
            logger_1.logger.error('Startup: failed to process pending withdrawals', err);
        }
        ...
        setInterval(async () => {
            try {
                ...
                for (const row of expired) {
                    try {
                        ...
                        if (nonceUsed) {
                            ...
                            await dbService.markPendingWithdrawalCompleted(row.wallet_address, BigInt(row.nonce));
                            completedCount++;
                            logger_1.logger.warn('Expired pending withdrawal was actually completed on-chain — marked completed (no refund)', {
                        ...
                        logger_1.logger.error('Failed to check nonce on-chain for pending withdrawal — skipping (will retry)', {
                            address: row.wallet_address,
                            nonce: row.nonce,
                            error: rpcErr instanceof Error ? rpcErr.message : String(rpcErr),
                        });
                ...
                if (refundedCount > 0) {
                    logger_1.logger.info(`Expired ${refundedCount} pending withdrawal(s) and refunded balances`);
                }
                if (completedCount > 0) {
                    logger_1.logger.info(`Marked ${completedCount} pending withdrawal(s) as completed (on-chain nonce used, no refund)`);
                }
            }
            catch (err) {
        ...
        setInterval(async () => {
            ...
            try {
                ...
                logger_1.logger.error('Hot withdrawal queue worker error', err);
            }
        ...
        setInterval(async () => {
            try {
                ...
                for (const job of jobs) {
                    ...
                    try {
                        const receipt = await publicClient.getTransactionReceipt({ hash: job.tx_hash });
                        if (receipt) {
                            if (receipt.status === 'success') {
                                    ...
                                    if (walletClient?.account) {
                                        try {
                                            const [distRecipient, burnAddr, platformRecipient, lpRecipient] = await Promise.all([
                                                publicClient.readContract({ address: blackjackContractAddress, abi: blackjack_1.blackjackAbi, functionName: 'distributionRecipient' }),
                                                publicClient.readContract({ address: blackjackContractAddress, abi: blackjack_1.blackjackAbi, functionName: 'burnAddress' }),
                                                publicClient.readContract({ address: blackjackContractAddress, abi: blackjack_1.blackjackAbi, functionName: 'platformFeeRecipient' }),
                                                publicClient.readContract({ address: blackjackContractAddress, abi: blackjack_1.blackjackAbi, functionName: 'lpDistributionRecipient' }),
                                            ]);
                                            ...
                                            const send = async (to, amount, label) => {
                                                if (amount <= 0n || !to || to === '0x0000000000000000000000000000000000000000')
                                                    return;
                                                try {
                                                    ...
                                                    logger_1.logger.info('Hot withdrawal fee sent', { jobId: job.id, to: label, amount: amount.toString() });
                                                }
                                                catch (e) {
                                                    logger_1.logger.error('Hot withdrawal fee transfer failed', { jobId: job.id, to: label, error: e });
                                                }
                                            };
                                            ...
                                            logger_1.logger.error('Hot withdrawal fee distribution failed (reading recipients)', { jobId: job.id, error: e });
                                        }
                                    }
                                }
                            }
                ...
                logger_1.logger.error('Hot withdrawal confirmation worker error', err);
            }
        }, 30_000); // every 30s
        ...
        setInterval(async () => {
            try {
                ...
                for (const row of pending) {
                    ...
                    if (blockNumber == null) {
                        try {
                            const receipt = await publicClient.getTransactionReceipt({ hash: row.tx_hash });
                    ...
                    if (confirmations >= row.confirmations_required) {
                        ...
                        if (credited) {
                            logger_1.logger.info('Deposit confirmed and credited', { wallet: row.wallet_address, txHash: row.tx_hash, confirmations });
                        }
                    }
                }
            }
            catch (err) {
                logger_1.logger.error('Pending deposits confirmation worker error', err);
            }
        }, 10_000); // every 10s
        ...
        const runContractSnapshot = async () => {
            try {
                ...
                logger_1.logger.error('Contract daily snapshot error', err);
            }
        };
        runContractSnapshot();
        ...
        app.post('/api/withdraw', async (req, res) => {
            try {
                ...
                if (amountBigInt < withdraw_sign_1.MIN_WITHDRAWAL_WEI) {
                    return res.status(400).json({ error: 'Amount required (min 1 MORBIUS)', minWei: withdraw_sign_1.MIN_WITHDRAWAL_WEI.toString() });
                }
                const walletClient = getHotWalletClient();
                if (!walletClient) {
                ...
                try {
                    ...
                    logger_1.logger.error('withdraw: failed to read hot wallet MORBIUS balance', {
                        error: rpcErr instanceof Error ? rpcErr.message : String(rpcErr),
                    });
                    return res.status(503).json({ error: 'Cannot verify withdrawal liquidity. Try again shortly.' });
                }
                if (hotMorbiusBalance < netToUser) {
                    logger_1.logger.warn('withdraw: hot wallet cannot cover net payout', {
                ...
                const jobId = await dbService.enqueueHotWithdrawal(normalizedAddress, amountBigInt, netToUser, feeAmount);
                ...
                logger_1.logger.error('Withdrawal enqueue error:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        app.get('/api/withdraw/status/:jobId', async (req, res) => {
            try {
                ...
                if (job.net_to_user_wei)
                    payload.netToUser = job.net_to_user_wei;
                return res.status(200).json(payload);
            }
            catch (error) {
                logger_1.logger.error('Withdraw status error:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        ...
        app.get('/api/withdraw/pending', async (req, res) => {
            try {
                ...
                logger_1.logger.error('Withdraw pending error:', error);
                res.status(500).json({ error: 'Internal server error' });
            }
        });
        // ─────────────────────────────────────────────────────────────────────────
...
let merkleDropsService = null;
let merkleDropsLPService = null;
