...
exports.ITEM_CATALOG = exports.MAX_SUPPLY = exports.FREE_VALUES = void 0;
exports.getItemKeyForValue = getItemKeyForValue;
exports.getLockedFields = getLockedFields;
exports.isAdminWallet = isAdminWallet;
exports.getUnlockedValuesPerField = getUnlockedValuesPerField;
exports.randomPlaceholderConfig = randomPlaceholderConfig;
...
// ─── Item factory ─────────────────────────────────────────────────────────────
function item(itemKey, displayName, tier, unlocks) {
    return {
        itemKey,
        displayName,
        tier,
        maxSupply: exports.MAX_SUPPLY[tier],
        unlocks,
        pricePls: PLS[tier],
...
const LEGACY_ROUND_GLASSES_RETIRED = [
    item('acc_round', 'Round Glasses', 'rare', [{ field: 'accessory', value: 'Round Glasses' }]),
];
const SHADES_VARIANT_ITEMS = Array.from({ length: 3 }, (_, i) => {
    const n = i + 1;
...
const LEGACY_SHADES_RETIRED_ITEMS = Array.from({ length: 7 }, (_, i) => {
    const n = i + 4;
    const value = `Shades V${n}`;
    return item(`acc_shades_v${n}`, value, 'uncommon', [{ field: 'accessory', value }]);
});
...
const HAT_VARIANT_NUMBERS = [1, 2, 3, 4, 5, 7, 8, 10];
const HAT_VARIANT_ITEMS = HAT_VARIANT_NUMBERS.map((n) => item(`hat_hat_v${n}`, `Hat V${n}`, 'uncommon', [{ field: 'hat', value: `Hat V${n}` }]));
const LEGACY_HAT_RETIRED_ITEMS = [
    item('hat_hat_v6', 'Hat V6', 'uncommon', [{ field: 'hat', value: 'Hat V6' }]),
    item('hat_hat_v9', 'Hat V9', 'uncommon', [{ field: 'hat', value: 'Hat V9' }]),
];
const HAT_COLOR_LEGENDARY_ITEMS = [
...
const LEGACY_NECKLACE_RETIRED_ITEMS = [
...
function adminShirtStyleItemKey(style) {
...
const SHIRT_STYLE_ITEMS = PAID_SHIRT_STYLES.map(style => item(adminShirtStyleItemKey(style), style, 'uncommon', [{ field: 'shirtStyle', value: style }]));
// ── Patterns (unlock url(#pattern) for skin, hair, AND shirt) ─────────────────
function patternItem(name, displayName, tier) {
    const val = `url(#${name})`;
...
const _valueToItemKey = new Map();
for (const ci of [...exports.ITEM_CATALOG, ...LEGACY_SHADES_RETIRED_ITEMS, ...LEGACY_HAT_RETIRED_ITEMS, ...LEGACY_ROUND_GLASSES_RETIRED, ...LEGACY_NECKLACE_RETIRED_ITEMS]) {
...
function getItemKeyForValue(field, value) {
    const freeSet = exports.FREE_VALUES[field];
 ...
 */
function getLockedFields(config, ownedItemKeys) {
    const locked = [];
    const fields = [
    ...
    for (const field of fields) {
        ...
        if (itemKey && !ownedItemKeys.has(itemKey)) {
            const catalogItem = exports.ITEM_CATALOG.find(i => i.itemKey === itemKey) ?? null;
 ...
 */
function getUnlockedValuesPerField(ownedItemKeys) {
    const result = {};
    for (const field of Object.keys(exports.FREE_VALUES)) {
        const freeSet = exports.FREE_VALUES[field];
        const values = new Set(freeSet ?? []);
        for (const catalogItem of exports.ITEM_CATALOG) {
            if (!ownedItemKeys.has(catalogItem.itemKey))
                continue;
            for (const u of catalogItem.unlocks) {
                if (u.field === field && u.value !== '__any_non_empty__')
                    values.add(u.value);
            }
        }
        result[field] = [...values];
 ...
 */
function randomPlaceholderConfig(ownedItemKeys) {
...
//# sourceMappingURL=cosmetics-catalog.js.map