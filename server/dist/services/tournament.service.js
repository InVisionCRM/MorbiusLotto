...
class TournamentService {
    ...
    getMinPlayersFromPrizeDistribution(type) {
        switch (type) {
            case 'winner_takes_all':
                return 1;
            case 'top_3':
     ...
     */
    async getTournamentStateByEntryId(entryId) {
        const query = `
      SELECT
        te.*,
        t.max_hands,
    ...
    async distributePrizes(tournamentId) {
        ...
        try {
            ...
            if (tournament.on_chain_tournament_id != null && !tournament.prize_token_address) {
                const { getMorbiusTournamentPrizePool } = await Promise.resolve().then(() => __importStar(require('../utils/morbius-tournament')));
            ...
            for (const row of prizesResult.rows) {
                ...
                if (prizeAmount > 0n) {
                    ...
                    if (useMorbiusPayout || useEscrowV2) {
                        ...
                        await (0, poker_chip_wallet_1.applyPokerChipDelta)(client, String(row.player_address).toLowerCase(), prizeAmount, 'tournament_prize', { type: 'tournament', id: tournamentId });
                    }
                    else {
            ...
            if (platformFeePercent > 0 && (platformWalletEnv || offChainPokerChips)) {
                ...
                if (platformFeeAmount > 0n) {
                    if (useMorbiusPayout || useEscrowV2) {
                        ...
                        await (0, poker_chip_wallet_1.applyPokerChipDelta)(client, platformWalletChips, platformFeeAmount, 'platform_fee', { type: 'tournament', id: tournamentId });
                    }
                    else if (platformWalletEnv) {
                        await client.query(`INSERT INTO players (wallet_address, balance) VALUES (LOWER($1), $2::NUMERIC)
               ON CONFLICT (wallet_address) DO UPDATE SET balance = players.balance + $2::NUMERIC`, [platformWalletEnv.toLowerCase(), platformFeeAmount.toString()]);
                    }
                }
            }
            if (creatorFeePercent > 0 && tournament.creator_address) {
                ...
                if (creatorFeeAmount > 0n) {
                    if (useMorbiusPayout || useEscrowV2) {
                        ...
                        await (0, poker_chip_wallet_1.applyPokerChipDelta)(client, tournament.creator_address.toLowerCase(), creatorFeeAmount, 'creator_fee', { type: 'tournament', id: tournamentId });
                    }
                    else {
    ...
    async createFreeroll(params) {
        ...
        if (params.startingChips !== exports.TOURNAMENT_CONFIG.STARTING_CHIPS) {
            throw new Error('Starting chips must be 5000');
        }
        if (params.maxHands !== exports.TOURNAMENT_CONFIG.MAX_HANDS) {
    ...
    async joinTournament(playerAddress, tournamentId, pinCode) {
        ...
        if (isOnChain) {
            ...
            const joined = await (0, morbius_tournament_1.hasJoinedMorbiusTournament)(tournament.on_chain_tournament_id, normalizedAddress);
            if (!joined) {
      ...
      `;
            ...
            const finalEntryCountResult = await client.query(`SELECT COUNT(*) AS c FROM tournament_entries WHERE tournament_id = $1`, [tournamentId]);
            ...
            if (tournament.status === 'registration' && finalEntryCount >= minPlayers) {
                await client.query(`UPDATE tournaments SET status = 'active', activated_at = NOW(), ends_at = NOW() + INTERVAL '24 hours' WHERE id = $1`, [tournamentId]);
                ...
                logger_1.logger.info('Tournament transitioned to active in DB (min players reached)', {
                    tournamentId,
     ...
     */
    async unregisterTournament(playerAddress, tournamentId) {
        const normalizedAddress = this.normalizeAddress(playerAddress);
        const tournament = await this.getTournament(tournamentId);
        if (!tournament) {
            throw new Error('Tournament not found');
        ...
        try {
            await client.query('BEGIN');
            await client.query(`UPDATE players SET balance = balance + $1::NUMERIC WHERE LOWER(wallet_address) = LOWER($2)`, [refundAmount.toString(), normalizedAddress]);
            await client.query(`UPDATE tournaments SET prize_pool = prize_pool - $1::NUMERIC WHERE id = $2`, [refundAmount.toString(), tournamentId]);
            await client.query(`DELETE FROM tournament_entries WHERE id = $1`, [entry.id]);
            await client.query('COMMIT');
            logger_1.logger.info('Player unregistered from tournament', {
                playerAddress: normalizedAddress,
                tournamentId,
    ...
    async handleFreerollStart(tournamentId) {
        ...
        const tournamentResult = await this.pool.query(`SELECT t.scheduled_start_at, t.duration_minutes, t.ends_at, t.prize_distribution_type, t.min_players,
              t.buy_in_amount, t.prize_token_address, t.on_chain_tournament_id, t.creator_address
       FROM tournaments t WHERE t.id = $1 AND t.tournament_type = 'freeroll'`, [tournamentId]);
        if (tournamentResult.rows.length === 0) {
        ...
        const entryCountResult = await this.pool.query(`SELECT COUNT(*) AS c FROM tournament_entries WHERE tournament_id = $1 AND registration_status IN ('registered', 'joined')`, [tournamentId]);
        const entryCount = parseInt(entryCountResult.rows[0]?.c ?? '0', 10);
        if (entryCount < minPlayers) {
            logger_1.logger.info('handleFreerollStart: insufficient players (%d < %d), cancelling tournament', entryCount, minPlayers);
     ...
     */
    async cancelTournamentDueToInsufficientPlayers(tournamentId) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            ...
            if (tournament.buy_in_amount > 0n) {
                for (const entry of entries) {
                    ...
                    if (refundAmount > 0n) {
                        await client.query(`UPDATE players SET balance = balance + $1::NUMERIC WHERE LOWER(wallet_address) = LOWER($2)`, [refundAmount.toString(), entry.player_address]);
                        logger_1.logger.info('Refunded buy-in for cancelled tournament (insufficient players)', {
                            tournamentId,
            ...
            await client.query(`UPDATE tournaments SET status = 'cancelled', ended_at = NOW(), current_phase = 'completed' WHERE id = $1`, [tournamentId]);
            if (tournament.prize_token_address) {
                const { cancelTournamentInEscrow } = await Promise.resolve().then(() => __importStar(require('../utils/escrow-payout')));
                const cancelResult = await cancelTournamentInEscrow(tournamentId);
                if (!cancelResult.success && cancelResult.error) {
                    logger_1.logger.warn('Failed to cancel tournament in escrow (insufficient players)', {
                        tournamentId,
            ...
            await client.query('COMMIT');
            logger_1.logger.info('Tournament cancelled due to insufficient players', { tournamentId, entryCount: entries.length });
        }
        catch (error) {
            await client.query('ROLLBACK');
            throw error;
     ...
     */
    async cancelTournament(tournamentId, cancellerAddress) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            const tournament = await this.getTournamentForUpdate(client, tournamentId);
            ...
            if (tournament.buy_in_amount > 0n) {
                if (isOnChain) {
                    ...
                    const { cancelMorbiusTournament, refundMorbiusTournamentPlayer } = await Promise.resolve().then(() => __importStar(require('../utils/morbius-tournament')));
                    ...
                    for (const entry of entries) {
                        ...
                        if (!refundResult.success) {
                            logger_1.logger.warn('Failed to refund player from on-chain tournament', {
                                tournamentId,
                                player: entry.player_address,
                            ...
                            logger_1.logger.info('Refunded player from on-chain tournament', {
                                tournamentId,
                                player: entry.player_address,
                    ...
                    for (const entry of entries) {
                        ...
                        if (refundAmount > 0n) {
                            await client.query(`UPDATE players SET balance = balance + $1::NUMERIC WHERE LOWER(wallet_address) = LOWER($2)`, [refundAmount.toString(), entry.player_address]);
                            logger_1.logger.info('Refunded buy-in for cancelled tournament', {
                                tournamentId,
                                player: entry.player_address,
                                amount: refundAmount.toString(),
                            });
            ...
            if (tournament.prize_token_address) {
                ...
                const cancelResult = await cancelTournamentInEscrow(tournamentId);
                if (!cancelResult.success && cancelResult.error) {
                    logger_1.logger.warn('Failed to cancel tournament in escrow (tournament marked cancelled in DB)', {
                        tournamentId,
                        error: cancelResult.error,
                    });
                }
            ...
            await client.query('COMMIT');
            logger_1.logger.info('Tournament cancelled', {
                tournamentId,
     ...
     */
    async handleTimeExpiredTournament(tournamentId) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            const tournament = await this.getTournamentForUpdate(client, tournamentId);
            ...
            const forfeitResult = await client.query(`UPDATE tournament_entries SET status = 'forfeited', finished_at = NOW() WHERE tournament_id = $1 AND status = 'playing' RETURNING id, player_address, total_buy_in`, [tournamentId]);
            const forfeitedEntries = forfeitResult.rows;
            // Count by status
            const countResult = await client.query(`SELECT status, COUNT(*) AS c FROM tournament_entries WHERE tournament_id = $1 GROUP BY status`, [tournamentId]);
            const completedCount = countResult.rows.find((r) => r.status === 'completed')?.c ?? 0;
            const bustedCount = countResult.rows.find((r) => r.status === 'busted')?.c ?? 0;
            const forfeitedCount = countResult.rows.find((r) => r.status === 'forfeited')?.c ?? 0;
            ...
            if (isOnChain && !useCustomToken) {
                ...
                for (const entry of entries) {
                    const refundResult = await refundMorbiusTournamentPlayer(tournament.on_chain_tournament_id, entry.player_address);
                    if (!refundResult.success) {
                        logger_1.logger.warn('Failed to refund player (everyone forfeited)', {
                            tournamentId,
                            player: entry.player_address,
                            error: refundResult.error,
                        });
            ...
            await client.query('COMMIT');
            logger_1.logger.info('Tournament cancelled (everyone forfeited), refunds processed', {
                tournamentId,
     ...
     */
    async creatorReclaimFunds(tournamentId, creatorAddress) {
        const client = await this.pool.connect();
        try {
            // Get tournament
            const tournament = await this.getTournament(tournamentId);
            ...
            if (isOnChain) {
                ...
                const { creatorReclaimFromEscrow } = await Promise.resolve().then(() => __importStar(require('../utils/escrow-payout')));
                const result = await creatorReclaimFromEscrow(tournamentId, creatorAddress);
                return result;
            }
        }
        catch (error) {
            logger_1.logger.error('Error in creatorReclaimFunds', { tournamentId, creatorAddress, error });
    ...
    async listCompletedTournaments(limit = 25, offset = 0) {
    ...
    async getTournamentResults(tournamentId) {
        ...
        const eRes = await this.pool.query(`SELECT te.id AS entry_id, te.player_address, te.final_rank, te.prize_won,
         ...
         ORDER BY
           ...
           te.bought_in_at ASC`, [tournamentId]);
        return {
            tournamentId: t.id,
            name: t.name,
    ...
    async getTournamentPokerStats(tournamentId) {
        const sumRes = await this.pool.query(`SELECT
       ...
       FROM poker_hands
       WHERE tournament_id = $1 AND completed_at IS NOT NULL`, [tournamentId]);
        const sum = sumRes.rows[0] ?? {};
        const biggestPotRes = await this.pool.query(`SELECT id, hand_number, pot_amount, completed_at, table_id
         FROM poker_hands
        ...
        const rakeRes = await this.pool.query(`SELECT COALESCE(SUM(rake_paid), 0)::bigint AS total_rake
         FROM poker_hand_players hp
         JOIN poker_hands h ON h.id = hp.hand_id
         WHERE h.tournament_id = $1`, [tournamentId]);
        return {
    ...
    async getTournamentHands(tournamentId, limit = 50, offset = 0, playerAddress) {
