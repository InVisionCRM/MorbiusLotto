...
function formatWei(wei) {
    try {
        const n = Number((0, viem_1.formatEther)((0, safe_bigint_1.toBigIntSafe)(wei)));
...
class DatabaseService {
    ...
    async deductAndCreatePendingWithdrawal(walletAddress, nonce, amount, expiresAt) {
        ...
        return this.withTransaction(async (client) => {
         ...
         WHERE LOWER(wallet_address) = LOWER($1) AND balance >= $2::NUMERIC
         RETURNING balance`, [normalizedAddress, amount.toString()]);
     ...
     */
    async markPendingWithdrawalCompleted(walletAddress, nonce, txHash) {
        const normalizedAddress = this.normalizeAddress(walletAddress);
        const query = `
    ...
    async listPendingDeposits(limit = 25, offset = 0) {
        const safeLimit = Math.min(Math.max(Number(limit) || 25, 1), 100);
        const safeOffset = Math.max(Number(offset) || 0, 0);
        const result = await this.pool.query(`SELECT id, wallet_address, amount_wei::TEXT AS amount_wei, tx_hash, status, created_at
       FROM pending_deposits
    ...
    async listPendingWithdrawals(limit = 25, offset = 0) {
        const safeLimit = Math.min(Math.max(Number(limit) || 25, 1), 100);
        const safeOffset = Math.max(Number(offset) || 0, 0);
        const result = await this.pool.query(`SELECT id, wallet_address, amount::TEXT AS amount, tx_hash, nonce::TEXT AS nonce, status, created_at
       FROM pending_withdrawals
       ORDER BY created_at DESC
       LIMIT $1 OFFSET $2`, [safeLimit, safeOffset]);
        return result.rows.map((r) => ({
     ...
     */
    async createGameHandInTx(client, gameId, handData) {
        const query = `
      INSERT INTO game_hands (
    ...
    `;
        ...
        if (!Array.isArray(rawHands) || rawHands.length === 0)
            return [];
        return rawHands.map((h, idx) => ({
            id: `${row.id}-${idx}`,
            game_id: gameId,
            hand_index: idx,
    ...
    async getBlackjackMultiRoundSeatWithRound(seatId) {
    ...
    async getBlackjackMultiRoundWithSeats(roundId) {
        ...
        const seatsResult = await this.pool.query(`SELECT * FROM blackjack_multi_round_seats WHERE round_id = $1 ORDER BY seat_position ASC`, [id]);
    ...
    async getProfile(walletAddress) {
        ...
        if (!row)
            return null;
        const avatarConfig = row.avatar_config != null && typeof row.avatar_config === 'object' ? row.avatar_config : null;
        return {
     ...
     */
    async setDefaultAvatarIfNull(walletAddress, avatarConfig) {
        const normalized = this.normalizeAddress(walletAddress);
       ...
       ON CONFLICT (wallet_address) DO UPDATE SET
         ...
         updated_at = NOW()`, [normalized, configJson]);
    }
    /** Explicitly update social/bio fields — allows clearing (pass empty string → stored as null). */
    async updateProfileSocial(walletAddress, bio, xHandle, tgHandle) {
        const normalized = this.normalizeAddress(walletAddress);
        await this.pool.query(`UPDATE chat_display_names
       SET bio = $2, x_handle = $3, tg_handle = $4, updated_at = NOW()
       WHERE wallet_address = $1`, [normalized, bio || null, xHandle || null, tgHandle || null]);
    }
    ...
    async createReport(data) {
        const result = await this.pool.query(`INSERT INTO user_reports (wallet_address, category, description, page_url, user_agent, balance_snapshot, recent_errors)
    ...
    async getReports(status, limit = 200) {
        const params = [limit];
        const where = status ? `WHERE status = $2` : '';
        if (status)
            params.push(status);
        const result = await this.pool.query(`SELECT id, wallet_address, category, description, page_url, user_agent,
    ...
    async updateReportStatus(id, status) {
        const result = await this.pool.query(`UPDATE user_reports SET status = $2 WHERE id = $1 RETURNING id`, [id, status]);
        return result.rows.length > 0;
    }
    /** How many reports has this wallet submitted in the last N minutes (rate limiting). */
    async getRecentReportCountByWallet(walletAddress, windowMinutes) {
        const normalized = this.normalizeAddress(walletAddress);
        const result = await this.pool.query(`SELECT COUNT(*) FROM user_reports
       WHERE wallet_address = $1 AND created_at > NOW() - INTERVAL '1 minute' * $2`, [normalized, windowMinutes]);
     ...
     */
    async logDeposit(walletAddress, amount, txHash, blockNumber, blockTimestamp) {
        const normalizedAddress = this.normalizeAddress(walletAddress);
        const createdAt = blockTimestamp
            ? new Date(Number(blockTimestamp) * 1000).toISOString()
            : null;
        await this.pool.query(`INSERT INTO player_deposits (wallet_address, amount, tx_hash, block_number, created_at)
       VALUES ($1, $2::NUMERIC, $3, $4, COALESCE($5::TIMESTAMPTZ, NOW()))
       ON CONFLICT (tx_hash) DO NOTHING`, [normalizedAddress, amount.toString(), txHash, blockNumber !== null ? Number(blockNumber) : null, createdAt]);
    }
    /**
     * Return a unified transaction history (deposits + withdrawals) for a wallet,
     * sorted newest-first.
     */
    async getPlayerTransactionHistory(walletAddress, limit = 50, offset = 0) {
        const normalizedAddress = this.normalizeAddress(walletAddress);
        const result = await this.pool.query(`SELECT type, amount::TEXT AS amount, status, tx_hash, created_at
       FROM (
         SELECT
           'deposit'    AS type,
           amount,
       ...
       ORDER BY created_at DESC
       LIMIT $2 OFFSET $3`, [normalizedAddress, limit, offset]);
        return result.rows.map((r) => ({
            type: r.type,
            amount: r.amount ?? '0',
    ...
    mapBlackjackSpWagerTierRow(r) {
        const tk = r.theme_kind;
        const themeKind = tk === 'image' || tk === 'video' ? tk : null;
        return {
            id: r.id,
            label: r.label,
    ...
    async listBlackjackSpWagerTiers(enabledOnly) {
        const where = enabledOnly ? 'WHERE enabled = true' : '';
        const r = await this.pool.query(`SELECT id, label, min_bet::TEXT, max_bet::TEXT, theme_kind, theme_id, sort_order, enabled, slug, created_at, updated_at
       FROM blackjack_sp_wager_tiers
       ${where}
       ORDER BY sort_order ASC, created_at ASC`);
        return r.rows.map((row) => this.mapBlackjackSpWagerTierRow(row));
    }
    async getBlackjackSpWagerTierById(id, enabledOnly) {
        const r = await this.pool.query(`SELECT id, label, min_bet::TEXT, max_bet::TEXT, theme_kind, theme_id, sort_order, enabled, slug, created_at, updated_at
       FROM blackjack_sp_wager_tiers
    ...
    async createBlackjackSpWagerTier(input) {
        const sortRes = await this.pool.query(`SELECT COALESCE(MAX(sort_order), -1) + 1 AS next_sort FROM blackjack_sp_wager_tiers`);
        ...
        const r = await this.pool.query(`INSERT INTO blackjack_sp_wager_tiers (label, min_bet, max_bet, theme_kind, theme_id, sort_order, slug, enabled)
       VALUES ($1, $2::NUMERIC, $3::NUMERIC, $4, $5, $6, $7, $8)
       RETURNING id, label, min_bet::TEXT, max_bet::TEXT, theme_kind, theme_id, sort_order, enabled, slug, created_at, updated_at`, [input.label, input.minBet.toString(), input.maxBet.toString(), themeKind, themeId, sortOrder, slug, enabled]);
        return this.mapBlackjackSpWagerTierRow(r.rows[0]);
    }
    async updateBlackjackSpWagerTier(id, updates) {
        const fields = [];
        const values = [];
        let i = 1;
        ...
        values.push(id);
        const r = await this.pool.query(`UPDATE blackjack_sp_wager_tiers SET ${fields.join(', ')} WHERE id = $${i}
       RETURNING id, label, min_bet::TEXT, max_bet::TEXT, theme_kind, theme_id, sort_order, enabled, slug, created_at, updated_at`, values);
        if (r.rows.length === 0)
            return null;
        return this.mapBlackjackSpWagerTierRow(r.rows[0]);
    }
    async deleteBlackjackSpWagerTier(id) {
    ...
    async getPokerPlayerHands(address, limit = 50, offset = 0) {
    ...
    async getPokerPlayerStats(address, scope = 'cash') {
        const normalized = this.normalizeAddress(address);
        const emptyResponse = {
            total_hands: 0,
            hands_won: 0,
        ...
        const scopeFilter = scope === 'cash' ? 'AND h.tournament_id IS NULL'
    ...
    `;
        ...
        const tournamentHandsRes = await this.pool.query(`SELECT COUNT(*)::INT AS n
         FROM poker_hand_players p
         JOIN poker_hands h ON h.id = p.hand_id
        WHERE LOWER(p.player_address) = LOWER($1)
          AND h.completed_at IS NOT NULL
          AND h.tournament_id IS NOT NULL`, [normalized]);
        const tournamentHands = Number(tournamentHandsRes.rows[0]?.n ?? 0);
        // Winning hand breakdown, same scope as primary stats
        const breakdownRes = await this.pool.query(`SELECT p.hand_name, COUNT(*)::INT AS count
         FROM poker_hand_players p
         JOIN poker_hands h ON h.id = p.hand_id
        WHERE LOWER(p.player_address) = LOWER($1)
          AND h.completed_at IS NOT NULL
        ...
        const winningHandBreakdown = breakdownRes.rows.map((r) => ({
    ...
    `;
        ...
        const outcomes = orderedResult.rows.map((r) => Number(r.outcome));
        let currentStreak = 0;
        let bestStreak = 0;
        let run = 0;
    ...
    async getPokerPlayerTableStats(tableId, address) {
        const normalized = this.normalizeAddress(address);
        const aggQuery = `
      SELECT
        COUNT(*)::INT AS total_hands,
    ...
    `;
        ...
        const handsHistory = orderedResult.rows.map((r) => ({
            hand_number: r.hand_number,
            completed_at: r.completed_at ? new Date(r.completed_at).toISOString() : '',
    ...
    async getPokerHandDetail(handId, playerAddress) {
       ...
       WHERE h.id = $1 AND h.completed_at IS NOT NULL`, [handId]);
        ...
        const actionsResult = await this.pool.query(`SELECT street, player_address, action, amount::TEXT AS amount
       FROM poker_hand_actions WHERE hand_id = $1 ORDER BY "order" ASC`, [handId]);
        const holeResult = await this.pool.query(`SELECT cards FROM poker_hand_hole_cards WHERE hand_id = $1 AND LOWER(player_address) = LOWER($2)`, [handId, normalized]);
        ...
        return {
            ...
            actions: actionsResult.rows.map((r) => ({
                street: r.street,
    ...
    async getPokerTableDashboardStats(tableId) {
        // Table info
        const tableResult = await this.pool.query(`SELECT id, small_blind::TEXT, big_blind::TEXT, max_seats, hand_number,
       ...
       FROM poker_tables WHERE id = $1`, [tableId]);
        ...
        // Current seats
        const seatsResult = await this.pool.query(`SELECT position, player_address, stack::TEXT, status,
    ...
    `, [tableId]);
        return {
            ...
            player_stats: playerStatsResult.rows.map((r) => ({
                player_address: r.player_address,
            ...
            recent_hands: recentResult.rows.map((r) => ({
                id: r.id,
                hand_number: r.hand_number,
    ...
    // ── Follow system ──────────────────────────────────────────────────────────
    async followPlayer(followerAddress, followingAddress) {
        const follower = followerAddress.toLowerCase();
        const following = followingAddress.toLowerCase();
        if (follower === following)
            throw new Error('Cannot follow yourself');
        await this.pool.query(`INSERT INTO player_follows (follower_address, following_address)
       VALUES ($1, $2)
       ON CONFLICT DO NOTHING`, [follower, following]);
    }
    async unfollowPlayer(followerAddress, followingAddress) {
        await this.pool.query(`DELETE FROM player_follows
       WHERE LOWER(follower_address) = LOWER($1)
         AND LOWER(following_address) = LOWER($2)`, [followerAddress, followingAddress]);
    }
    async isFollowing(followerAddress, followingAddress) {
        const result = await this.pool.query(`SELECT 1 FROM player_follows
       WHERE LOWER(follower_address) = LOWER($1)
         AND LOWER(following_address) = LOWER($2)`, [followerAddress, followingAddress]);
        return result.rowCount > 0;
    }
    async getFollowCounts(address) {
        const result = await this.pool.query(`SELECT
         ...
         (SELECT COUNT(*) FROM player_follows WHERE LOWER(follower_address)  = LOWER($1)) AS following_count`, [address]);
        return {
            followerCount: Number(result.rows[0]?.follower_count ?? 0),
            followingCount: Number(result.rows[0]?.following_count ?? 0),
        };
    }
    async getFollowers(address, limit = 50, offset = 0) {
        const result = await this.pool.query(`SELECT pf.follower_address AS address,
              cdn.display_name,
              cdn.avatar_config
    ...
    async getFollowing(address, limit = 50, offset = 0) {
        const result = await this.pool.query(`SELECT pf.following_address AS address,
              cdn.display_name,
              cdn.avatar_config
       FROM player_follows pf
