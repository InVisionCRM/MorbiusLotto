...
export const CYPRESS_DEFAULT_POKER_BOT_ADDRESSES = [
...
export function parsePokerBotWalletCsv(input?: string): string[] {
  if (!input) return [];
  return [
    ...new Set(
      input
 ...
 */
export function getPokerBotWalletAddressList(opts?: PokerBotWalletListOptions): string[] {
  const strict =
    opts?.server === true &&
    ['1', 'true', 'yes'].includes(
      String(process.env.POKER_SERVER_BOT_STRICT_ADDRESSES ?? '').toLowerCase(),
    );
  ...
  return CYPRESS_DEFAULT_POKER_BOT_ADDRESSES.map((a) => a.toLowerCase());
}
