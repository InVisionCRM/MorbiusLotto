  ...
  try {
    ...
    app.get('/api/poker/top-players', async (req, res) => {
      try {
        ...
        const requester =
          typeof req.query.address === 'string' && /^0x[a-fA-F0-9]{40}$/.test(req.query.address)
            ? req.query.address
            : null;
    ...
    app.get('/api/poker/verify/:handId', async (req, res) => {
      try {
        ...
        const handRow = await dbService.getPool().query(
          `SELECT id, table_id, tournament_id, hand_number, completed_at,
        ...
        const serverSeed: string = h.server_seed;
        const clientSeed: string = h.client_seed ?? '';
        const serverSeedHash: string = h.server_seed_hash;
        ...
        const actionRows = await dbService.getPool().query(
          `SELECT "order", player_address, street, action, amount
             FROM poker_hand_actions WHERE hand_id = $1 ORDER BY "order" ASC`,
          [handId],
        );
        const actions = actionRows.rows.map((r: any) => ({
        ...
        logger.error('Error verifying poker hand:', error);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
    ...
    const blackjackSpWagerTiersSchemaErrorResponse = (res: express.Response, error: unknown, context: string): boolean => {
      const err = error as { code?: string; message?: string };
      const msg = typeof err.message === 'string' ? err.message : '';
      if (err.code === '42P01' || /relation ["']?blackjack_sp_wager_tiers["']? does not exist/i.test(msg)) {
        logger.error(`${context}: blackjack_sp_wager_tiers missing`, { error });
        res.status(503).json({
          error:
            'Run migration 109 from repo root: node server/run-migration.js migrations/109_blackjack_sp_wager_tiers.sql',
        });
        return true;
      }
      if (err.code === '42703' && msg.includes('blackjack_sp_wager_tiers')) {
        logger.error(`${context}: blackjack_sp_wager_tiers column mismatch`, { error });
        res.status(503).json({
          error:
            'blackjack_sp_wager_tiers schema mismatch. Apply migrations 109 and 111 (see server/migrations/).',
        });
        return true;
      }
      if (/value too long/i.test(msg) && /label/i.test(msg)) {
        res.status(400).json({
          error:
            'Label column too small. Run: node server/run-migration.js migrations/111_blackjack_sp_wager_tiers_label_widen.sql',
        });
        return true;
      }
      return false;
    ...
    app.get('/api/blackjack/wager-tiers', async (_req, res) => {
      try {
        ...
        if (blackjackSpWagerTiersSchemaErrorResponse(res, error, 'GET /api/blackjack/wager-tiers')) return;
        logger.error('Error fetching blackjack wager tiers:', error);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
    ...
    const mapSpWagerTierAdmin = (t: BlackjackSpWagerTierRow) => ({
      id: t.id,
      label: t.label,
      minBet: t.min_bet,
      maxBet: t.max_bet,
    ...
    const parseSpTierBounds = (
      minBetRaw: unknown,
      maxBetRaw: unknown,
    ): { ok: true; minBet: bigint; maxBet: bigint } | { ok: false; error: string } => {
      ...
      if (minBet < 0n || maxBet <= 0n) return { ok: false, error: 'Bet bounds must be positive' };
      if (minBet > maxBet) return { ok: false, error: 'minBet cannot exceed maxBet' };
    ...
    app.get('/api/admin/bj-single/wager-tiers', async (_req, res) => {
      try {
        ...
        if (blackjackSpWagerTiersSchemaErrorResponse(res, error, 'GET /api/admin/bj-single/wager-tiers')) return;
        logger.error('Error listing BJ single wager tiers:', error);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
    ...
    app.post('/api/admin/bj-single/wager-tiers', async (req, res) => {
      try {
        ...
        let themeKind: 'image' | 'video' | null | undefined = undefined;
        ...
        if (blackjackSpWagerTiersSchemaErrorResponse(res, error, 'POST /api/admin/bj-single/wager-tiers')) return;
        logger.error('Error creating BJ single wager tier:', error);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
    ...
    app.patch('/api/admin/bj-single/wager-tiers/:tierId', async (req, res) => {
      try {
        ...
        const body = req.body as Record<string, unknown>;
        const updates: Parameters<typeof dbService.updateBlackjackSpWagerTier>[1] = {};
        ...
        if (body.minBet !== undefined || body.maxBet !== undefined) {
          ...
          const minStr =
            body.minBet !== undefined ? String(body.minBet as string | number) : existing.min_bet;
          const maxStr =
            body.maxBet !== undefined ? String(body.maxBet as string | number) : existing.max_bet;
        ...
        if (body.themeKind === null || body.themeKind === '') {
          ...
          if (typeof body.themeId === 'string') updates.themeId = body.themeId.trim() || null;
        } else if (body.themeKind !== undefined) {
          res.status(400).json({ error: 'themeKind must be image, video, or null' });
          return;
        ...
        if (blackjackSpWagerTiersSchemaErrorResponse(res, error, 'PATCH /api/admin/bj-single/wager-tiers')) return;
        logger.error('Error updating BJ single wager tier:', error);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
    ...
    app.delete('/api/admin/bj-single/wager-tiers/:tierId', async (req, res) => {
      try {
        ...
        if (blackjackSpWagerTiersSchemaErrorResponse(res, error, 'DELETE /api/admin/bj-single/wager-tiers')) return;
        logger.error('Error deleting BJ single wager tier:', error);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
