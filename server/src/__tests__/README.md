...
1. Migration 063 and 064 applied (`poker_tables.tournament_mode`, `tournaments.game_type`, `poker_tournament_seats` table, `poker_tournament_registrations` view)
...
| **9 — regression** | Blackjack `createTournament` still defaults to `game_type=blackjack`. Cash game tables still appear in `listTables`. Tournament tables do NOT appear in `listTables`. `cancelPokerTournament` refunds all players. |
...
| `relation "poker_tournament_registrations" does not exist` or column error | Run migration 064: `node server/run-migration.js migrations/064_poker_tournament_view_scheduled_start.sql` |
