//cd /Users/kyle/MORBlotto/contracts && npx hardhat run scripts/lottery/fund/fund-instant-lottery.js --network pulsechain

const hre = require("hardhat");

async function main() {
  console.log("Funding Instant Lottery contract with MORBIUS…");
  ...
  const LOTTERY_INSTANT_ADDRESS =
    ...
    "0x884843787be5c0387F38722d9e4F2ab1E93c25D8";
  ...
  console.log("\nConfig:");
  console.log("LOTTERY_INSTANT_ADDRESS:", LOTTERY_INSTANT_ADDRESS);
  ...
  console.log("\nApproving…");
  await (await MORBIUS.approve(LOTTERY_INSTANT_ADDRESS, FUNDING_AMOUNT, gasOverrides)).wait();
  const allowance = await MORBIUS.allowance(deployer.address, LOTTERY_INSTANT_ADDRESS);
  ...
  const InstantLottery = await hre.ethers.getContractAt(
    "InstantLottery6of55",
    LOTTERY_INSTANT_ADDRESS
  );

  try {
    const tx = await InstantLottery.fundContract(FUNDING_AMOUNT, gasOverrides);
    await tx.wait();
    const reserve = await InstantLottery.contractReserve();
    ...
    if (err.data) {
      try {
        const iface = InstantLottery.interface;
