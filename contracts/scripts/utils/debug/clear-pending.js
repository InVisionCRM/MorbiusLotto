 ...
 * Uses BACKUP_PRIVATE_KEY from .env if set, otherwise PRIVATE_KEY (same wallet that sent the stuck tx).
 *
 * After running: the stuck tx is replaced/cancelled. Redeploy or retry your original action.
 *
 * Usage (from repo root or contracts):
 *   cd contracts && npx hardhat run scripts/utils/debug/clear-pending.js --network pulsechain
 *
...
async function main() {
  ...
  const key = process.env.BACKUP_PRIVATE_KEY || process.env.PRIVATE_KEY;
  if (!key || !key.startsWith("0x")) {
    console.error("Set BACKUP_PRIVATE_KEY or PRIVATE_KEY in .env (0x-prefixed hex).");
    process.exit(1);
  }
  const deployer = new hre.ethers.Wallet(key.trim(), provider);
  console.log("Clearing stuck transactions for:", deployer.address, process.env.BACKUP_PRIVATE_KEY ? "(BACKUP_PRIVATE_KEY)" : "");
