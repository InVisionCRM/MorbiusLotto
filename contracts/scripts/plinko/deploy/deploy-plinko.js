...
async function main() {
  ...
  console.log(`   npx hardhat verify --network ${hre.network.name} ${addr} "${MORBIUS_TOKEN}" "${WPLS_TOKEN}" "${PULSEX_ROUTER}" "${MIN_WAGER}" "${MAX_WAGER}" "${PLS_TREASURY}" "${DISTRIBUTION_RECIPIENT}" "${BURN_ADDRESS}" "${PLATFORM_FEE_RECIPIENT}" "${LP_DISTRIBUTION_RECIPIENT}"`);
}
