/**
 * Client-side Texas Hold'em hand evaluation for 5-7 cards.
 * Ported from server/src/services/poker-hand-eval.ts with plain enum
 * so it works in client bundles.
 *
...
function compareHands(a: RankedHand, b: RankedHand): number {
...
/** Preflop-only: returns "Pair" if hole cards share a rank, otherwise null. */
export function evaluateHoleCards(cards: number[]): string | null {
  if (cards.length !== 2) return null;
  return rankOf(cards[0]) === rankOf(cards[1]) ? 'Pair' : null;
}
