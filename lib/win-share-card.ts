 ...
 */
export function drawWinCard(ctx: CanvasRenderingContext2D, width: number, height: number, data: WinCardData): void {
  const s = width / REF_W
  ctx.save()
  ctx.scale(s, s)
...
export function renderWinCardBlob(data: WinCardData): Promise<Blob | null> {
  const canvas = document.createElement('canvas')
  canvas.width = REF_W
  canvas.height = REF_H
  const ctx = canvas.getContext('2d')
  if (!ctx) return Promise.resolve(null)
  drawWinCard(ctx, REF_W, REF_H, data)
