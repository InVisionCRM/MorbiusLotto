/**
 * Snapshot exclusion list: all LP and game contract addresses from ALL_DEPLOYMENTS.MD
 * plus current contracts from lib/contracts. Use for both:
 * - Merkle (holders) drop blocklist
 * - Merkle LP drop blocklist
 */
import {
  PLINKO_ADDRESS,
  KENO_ADDRESS,
...
/** All contract addresses from ALL_DEPLOYMENTS.MD (Deployer 1, 2, 3, Other) + current lib/contracts. Lowercase for consistent comparison. */
export const SNAPSHOT_EXCLUSION_CONTRACTS: string[] = [
...
export const SNAPSHOT_EXCLUSION_SET = new Set(SNAPSHOT_EXCLUSION_CONTRACTS);
