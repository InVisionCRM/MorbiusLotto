 ...
 *
 * Public (no auth, no session) view of a single completed Crash round. Pulls
 * the verification payload from `GET /api/arcade/crash/verify/:id`, then
 ...
 *      → float r ∈ [0,1) → crashX100 = max(100, floor(((1 - houseEdgeBp/1e4) /
 *      r) × 100))   — and that equals the published crashX100.
 *
 * Note: Crash uses r (not 1-r) in the denominator, so the distribution is
 * inverted relative to Limbo — high r = low crash, low r = high crash.
 *
 * Style mirrors the Limbo verifier.
 */
...
/**
 * Re-derive the crash point from the revealed seeds.
 * Formula: crashX100 = max(100, floor((1 - houseEdgeBp/10000) / r × 100))
 * where r = bytesToFloat(HMAC-SHA256(serverSeed, `${clientSeed}:${nonce}:0`)[0..3])
 */
async function deriveCrashX100(
  ...
  try {
    ...
    // Crash formula: divide by r (not 1-r like Limbo)
    const raw = houseFactor / safe;
    const crashX100 = Math.max(100, Math.floor(raw * 100));
...
export default function TgVerifyCrashRoundPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (h == null) { setHashState('no-crypto'); return; }
      setLocalHash(h);
      setHashState(
    ...
    >
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-5 px-4 py-6 pb-16">
        <header className="flex flex-col gap-1">
          ...
          >
            Crash Round Verifier
          </h1>
          <p className="text-[13px] text-white/60">
            Independently confirm that this Crash round&apos;s crash point was fixed before the
            bet was placed. Both the seed commitment and the crash-point math are re-derived in
            your browser — no need to trust the server.
          </p>
        </header>
          ...
          <>
            ...
            >
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[12px]">
                ...
                <dd className="text-right font-semibold text-cyan-300 tabular-nums">
                  {payload.autoCashoutX100 != null
                    ? formatMultiplierX100(payload.autoCashoutX100)
                    : '—'}
                </dd>
                ...
                >
                  {formatMultiplierX100(payload.crashX100)}
                </dd>
                <dt className="text-white/50">Cashed out at</dt>
                <dd className="text-right font-semibold tabular-nums text-cyan-300">
                  {payload.cashoutX100 != null
                    ? formatMultiplierX100(payload.cashoutX100)
                    : '—'}
                </dd>
              ...
              <p className="border-t border-cyan-500/10 pt-3 text-[12px] leading-snug text-white/55">
                Before each round the server publishes{' '}
                <span className="text-cyan-300">sha256(serverSeed)</span>. After the round the
                seed itself is revealed — your browser re-hashes it, re-runs the HMAC roll, and
                recomputes the crash point independently.
              </p>
            </div>
            ...
            <Section title="Step 2 · Crash-point re-derivation">
              <div className="flex items-center justify-end">
                <StatusBadge
                  ...
                  label={
                    crashState === 'verified'
                      ? `Matches ${formatMultiplierX100(payload.crashX100)}`
                      : crashState === 'mismatch'
              ...
              >
                ...
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">derived crash point</span>
                  <code
                    className="text-cyan-200 tabular-nums"
                    style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}
                  >
                    {derived ? formatMultiplierX100(derived.crashX100) : '…'}
                  </code>
                </div>
            ...
            <footer className="pt-2 text-center text-[11px] text-white/40">
              ...
              <code style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}>
                GET /api/arcade/crash/verify/{shortId(payload.roundId)}
              </code>
            </footer>
          </>
        )}
