...
/**
 * /tg/verify/limbo/[roundId] — Telegram-friendly provably-fair Limbo verifier.
 *
 * Public (no auth, no session) view of a single completed Limbo round. Pulls
 * the verification payload from `GET /api/arcade/limbo/verify/:id`, then
 ...
 *      (1 - r)) × 100))   — and that equals the published resultX100.
 *
 * If both checks pass, the round was honest: the crash point was fixed before
 * the round began. Visual style mirrors `/tg/verify/[handId]` so the two pages
...
// Re-derive the round's float and crash point from the revealed seeds. Mirrors
// `pfService.hmacByteStream(serverSeed, clientSeed, nonce, 0)` +
// `pfService.bytesToFloat(bytes)` + `crashPointFromFloat` on the server.
...
export default function TgVerifyLimboRoundPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/arcade/limbo/verify/${encodeURIComponent(roundId)}`);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (!d) {
        setCrashState('no-crypto');
        return;
      }
      setDerived(d);
      setCrashState(d.crashX100 === payload.resultX100 ? 'verified' : 'mismatch');
    })();
    return () => {
      cancelled = true;
    };
    ...
    >
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-5 px-4 py-6 pb-16">
        <header className="flex flex-col gap-1">
          ...
          >
            Limbo Round Verifier
          </h1>
          <p className="text-[13px] text-white/60">
            Independently confirm that this Limbo round&apos;s crash point was fixed before the
            roll. Both the seed commitment and the crash-point math are re-derived in your
          ...
          <>
            ...
            >
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[12px]">
                ...
                <dd className="text-right font-semibold text-cyan-300 tabular-nums">
                  {formatMultiplierX100(payload.targetX100)}
                </dd>
                ...
                >
                  {formatMultiplierX100(payload.resultX100)}
                </dd>
              ...
              <p className="border-t border-cyan-500/10 pt-3 text-[12px] leading-snug text-white/55">
                ...
                the HMAC roll, and recomputes the crash point.
              </p>
            </div>
            ...
            <Section title="Step 2 · Crash-point re-derivation">
              <div className="flex items-center justify-end">
                <StatusBadge
                  ...
                  label={
                    crashState === 'verified'
                      ? `Matches ${formatMultiplierX100(payload.resultX100)}`
                      : crashState === 'mismatch'
              ...
              >
                ...
                <div className="flex justify-between gap-3">
                  <span className="text-white/50">derived crash</span>
                  <code
                    className="text-cyan-200 tabular-nums"
                    style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}
                  >
            ...
            <footer className="pt-2 text-center text-[11px] text-white/40">
              ...
              <code style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}>
                GET /api/arcade/limbo/verify/{shortId(payload.roundId)}
              </code>
            </footer>
          </>
        )}
