...
/**
 * Proxy all /api/arcade/andar-bahar/* requests to the Express backend.
 *
 * The Andar Bahar game (components/AndarBahar/AndarBaharGame.tsx) calls
 * /api/arcade/andar-bahar/{info,play,history,recent,leaderboard,verify/:id} as
...
async function proxy(req: NextRequest, path: string[]): Promise<NextResponse> {
  ...
  const targetPath = `/api/arcade/andar-bahar/${path.join('/')}${qs ? `?${qs}` : ''}`;
