...
/**
 * Proxy all /api/arcade/cascade/* requests to the Express backend.
 *
 * The Cascade game (components/Cascade/CascadeGame.tsx) calls
 * /api/arcade/cascade/{info,play,history,recent,leaderboard,verify/:id} as
...
async function proxy(req: NextRequest, path: string[]): Promise<NextResponse> {
  ...
  const targetPath = `/api/arcade/cascade/${path.join('/')}${qs ? `?${qs}` : ''}`;
