        ...
        <main className="container mx-auto px-4 py-8 max-w-3xl">
          ...
          <div className="mb-4">
            ...
            >
              ...
              Filters
              {activeFilterCount > 0 && (
                <span className="bg-cyan-600 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] font-bold">{activeFilterCount}</span>
              )}
            </button>
            {showFilters && (
              <div className="mt-2 flex flex-wrap gap-2 p-3 bg-slate-800/40 border border-slate-700 rounded-lg">
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1">
                    {([['all', 'All'], ['open', 'Open'], ['active', 'In Progress']] as const).map(([val, label]) => (
                      <button
                        key={val}
                        onClick={() => setStatusFilter(val)}
                        className={`px-2.5 py-1 rounded text-xs transition-colors ${
                          statusFilter === val ? 'bg-cyan-600 text-white' : 'bg-slate-700/60 text-white/60 hover:text-white'
                        }`}
                      >{label}</button>
                    ))}
                  </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1">
                    {([['all', 'Any'], ['has_seats', 'Available'], ['full', 'Full']] as const).map(([val, label]) => (
                      <button
                        key={val}
                        onClick={() => setSeatFilter(val)}
                        className={`px-2.5 py-1 rounded text-xs transition-colors ${
                          seatFilter === val ? 'bg-cyan-600 text-white' : 'bg-slate-700/60 text-white/60 hover:text-white'
                        }`}
                      >{label}</button>
                    ))}
                  </div>
                ...
                {activeFilterCount > 0 && (
                  <button
                    onClick={() => { setStatusFilter('all'); setSeatFilter('all'); }}
