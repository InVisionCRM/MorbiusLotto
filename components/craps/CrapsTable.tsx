...
export function CrapsTable({ bets, point, phase, activeChip, placeBet, isRolling }: Props) {
  ...
  const PlaceBox = ({ num, label, type }: { num: number; label: string; type: BetType }) => (
    ...
    >
      ...
      <span
        className={cn(
          'text-xl sm:text-2xl arc-mono font-bold',
  ...
  return (
    <div className="w-full h-full flex flex-col gap-2 select-none text-slate-200 relative">
      ...
      <div className="flex-1 grid grid-cols-1 sm:grid-cols-12 gap-2 mt-2">
        ...
        <div className="sm:col-span-8 flex flex-col gap-2">
          <div
            ...
            className={cn(
              'flex-1 min-h-[60px] bg-[#081420]/50 border border-dashed border-cyan-500/30 rounded-2xl flex items-center justify-center cursor-pointer hover:bg-cyan-500/10 transition-colors relative',
              phase === 'POINT' && 'opacity-50 cursor-not-allowed',
            )}
          >
            <h2 className="text-xl sm:text-3xl font-bold opacity-20 tracking-[0.5em] sm:tracking-[1em] arc-display text-cyan-300">
              DON&apos;T PASS
            </h2>
          ...
          <div
            onClick={() => click('FIELD')}
            className="h-24 sm:h-32 bg-[#081420]/40 border border-cyan-500/15 rounded-2xl p-2 sm:p-4 flex flex-col justify-between cursor-pointer hover:bg-cyan-500/10 transition-colors relative"
          >
            ...
            <div className="flex justify-between items-center px-4 sm:px-8 mt-2 sm:mt-0 arc-mono">
              <span className="text-xl font-bold text-slate-200">2</span>
              <span className="text-sm sm:text-xl font-medium opacity-60 text-slate-300">3 · 4 · 9 · 10 · 11</span>
        ...
        <div className="sm:col-span-4 bg-[#050E16]/70 border border-cyan-500/20 rounded-2xl p-2 sm:p-4 grid grid-cols-2 gap-2">
          <div
            onClick={() => click('ANY_7')}
            className="bg-[#081420]/60 rounded-lg flex flex-col items-center justify-center border border-cyan-500/15 cursor-pointer hover:bg-cyan-500/10 transition-colors relative min-h-[60px]"
          >
          ...
          <div className="col-span-2 bg-cyan-500/10 rounded-lg flex flex-col items-center justify-center border border-cyan-500/30 min-h-[40px] relative mt-2">
      ...
      <div
        onClick={() => click('PASS')}
        className={cn(
          'mt-2 h-16 sm:h-20 bg-cyan-500/10 border-2 border-cyan-400 rounded-2xl flex items-center justify-center relative overflow-hidden cursor-pointer hover:bg-cyan-500/20 transition-colors shadow-[0_0_28px_-10px_rgba(34,211,238,0.7)]',
          phase === 'POINT' && 'opacity-50 cursor-not-allowed',
        )}
      >
        <div className="absolute inset-0 opacity-20 flex justify-around items-center arc-display">
          <span className="text-2xl sm:text-4xl font-bold hidden sm:block text-cyan-300">PASS LINE</span>
          <span className="text-2xl sm:text-4xl font-bold text-cyan-300">PASS LINE</span>
          <span className="text-2xl sm:text-4xl font-bold hidden md:block text-cyan-300">PASS LINE</span>
        </div>
