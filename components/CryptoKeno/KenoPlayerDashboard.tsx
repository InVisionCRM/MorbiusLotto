...
interface KenoPlayerDashboardProps {
  playerAddress: string | null
}

export function KenoPlayerDashboard({ playerAddress }: KenoPlayerDashboardProps) {
  ...
  const cumulativeChartData = useMemo(() => {
    if (sortedForChart.length === 0) {
      return [
        ...
        {
          play: Number(totalPlays),
          date: 'Now',
          totalInvested: Math.floor(Number(totalWagered) / 1e18),
          totalWon: Math.floor(Number(totalWon) / 1e18),
        },
      ]
    }
    ...
    return points
  }, [sortedForChart, totalPlays, totalWagered, totalWon])
  ...
  const roi = totalWagered > 0n
    ? Number((profitLoss * 10000n) / totalWagered) / 100
    : 0
              ...
              <div className="h-[320px] w-full min-w-0">
                {cumulativeChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      <defs>
                        <linearGradient id="kenoColorInvested" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#a855f7" stopOpacity={0.8} />
                          <stop offset="95%" stopColor="#a855f7" stopOpacity={0.1} />
                        </linearGradient>
                        <linearGradient id="kenoColorWon" x1="0" y1="0" x2="0" y2="1">
                      ...
                      const dateStr =
                        r.timestamp != null
                          ...
                          : r.blockNumber != null
                            ...
                            : '—'
                      return (
                        <TableRow key={`${r.ticketId}-${r.transactionHash ?? i}`} className="border-white/5 hover:bg-white/5">
                          <TableCell className="text-white/80 text-xs font-mono py-2 whitespace-nowrap">
                            {dateStr}
                          </TableCell>
                          <TableCell className="py-2 text-center text-white/90 text-xs">{r.spotSize}</TableCell>
                          <TableCell className="py-2 text-center">
                            <span className="text-xs font-medium text-white">{r.hits}</span>
                          </TableCell>
