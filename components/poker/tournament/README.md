...
4. **Each completed hand** — `PokerGameService` post-hand callback invokes **`syncAfterHand(tableId, handNumber)`** (only while **`tournaments.status === 'active'`**). For all-in runouts (multiple streets dealt in one tick), `completed_at` is only set after the server's paced flop/turn/river/showdown broadcasts finish — see [`components/poker/README.md`](../README.md) → "All-in runout (server-driven)". `syncAfterHand` still fires from the 15s post-showdown timer body, so the elimination + blind-up sequence is unchanged; the only difference is the runout adds ~2–6 seconds of "reveal" wall-clock time before the timer is armed. A crash between `runout_resolved_at` and `completed_at` is recovered by `recoverStuckPostHandTables` before `syncAfterHand` runs.
