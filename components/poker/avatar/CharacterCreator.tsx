...
export default function CharacterCreator({ config: controlledConfig, onChange, initialConfig, displayName, onDisplayNameChange, compact = false }: CharacterCreatorProps) {
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-0">
      <div className={`w-full border-b border-zinc-800 bg-zinc-900/80 overflow-x-auto scrollbar-hide shrink-0 ${compact ? 'p-2' : 'p-4'}`}>
        <div className="flex space-x-1.5 max-w-full md:justify-center">
          {tabs.map(tab => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`relative flex items-center space-x-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap shrink-0 ${compact ? 'px-2.5 py-1.5' : 'px-4 py-2.5 rounded-xl text-sm'} ${isActive ? 'text-indigo-400' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'}`}
              >
                {isActive && (
                  <motion.div
      ...
      <div className="flex flex-col md:flex-row flex-1 min-h-0">
        <div className={`w-full md:w-2/5 bg-zinc-800/50 flex flex-col items-center justify-center relative border-b md:border-b-0 md:border-r border-zinc-800 shrink-0 ${compact ? 'p-3' : 'p-8'}`}>
          {displayName !== undefined && onDisplayNameChange ? (
            <input
              ...
              maxLength={32}
              className={`font-bold text-zinc-100 tracking-tight w-full max-w-[200px] text-center bg-transparent border-b-2 border-zinc-600 hover:border-zinc-500 focus:border-cyan-500 focus:outline-none placeholder:text-zinc-500 pb-1 transition-colors ${compact ? 'text-lg mb-3' : 'text-2xl mb-8'}`}
            />
          ) : (
            <h1 className={`font-bold text-zinc-100 tracking-tight ${compact ? 'text-lg mb-3' : 'text-2xl mb-8'}`}>Player Profile</h1>
          )}
          <AvatarPreview config={config} emotion={emotion} glassesAnimationKey={glassesAnimationKey} className={compact ? 'w-28 h-28' : undefined} />

          <div className={`text-center w-full ${compact ? 'mt-3' : 'mt-8'}`}>
            <button
              onClick={handleDealWithIt}
              className={`mx-auto flex items-center gap-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-100 rounded-lg font-medium transition-colors shadow-sm border border-zinc-700 hover:border-zinc-600 ${compact ? 'mb-3 px-2.5 py-1.5 text-xs' : 'mb-6 px-4 py-2 rounded-xl'}`}
            >
              <Glasses size={compact ? 12 : 16} />
              Shades
            </button>

            <p className={`text-zinc-400 ${compact ? 'text-xs mb-2' : 'text-sm mb-4'}`}>Test Emotions</p>
            <div className="flex justify-center gap-1.5 flex-wrap">
              {(['happy', 'sad', 'angry', 'surprised', 'wink'] as Emotion[]).map((emo) => (
                <button
                  key={emo}
                  onClick={() => playEmotion(emo)}
                  className={`rounded-lg font-medium capitalize transition-colors ${compact ? 'px-2 py-1 text-[10px]' : 'px-3 py-1.5 text-xs'} ${emotion === emo ? 'bg-indigo-500 text-white' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}
                >
                  {emo}
                </button>
              ))}
        ...
        <div className={`w-full md:w-3/5 flex-1 min-h-0 overflow-y-auto ${compact ? 'p-3' : 'p-6 sm:p-8'}`}>
