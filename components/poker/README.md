...
Organized index of all poker-related files: components, app routes, server logic, migrations, lib, and scripts. No code—reference only.
...
| `app/poker/designer/page.tsx` | Layout designer: drag-and-drop positions for table, seats, community cards, pot, action bar, chat; saves to localStorage. |
...
| File | Purpose |
|------|--------|
| `PokerTable.tsx` | Main table container: seats, board, pot; uses layout from `poker-layout` for positioning. |
| `PokerSeat.tsx` | Single seat: player address, stack, dealer/SB/BB/acting flags, hole cards or card backs, last action. |
...
- **Tooling**: `app/poker/designer` (layout), `server/src/scripts/poker-bot.ts` (bots).
