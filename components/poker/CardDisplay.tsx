...
// ── Card animation ─────────────────────────────────────────────────────────
//
// One animation, applied via plain CSS keyframes: a brief opacity fade-in
// (~220ms) when the card first mounts. No spring, no slide-in, no scale
// punch, no exit animation. The old framer-motion variant system (Spin
// Pitch / Shockwave Slam) was re-firing on parent re-renders and producing
// visible flicker; this is intentionally boring and tear-free.
//
// Per-card stagger for the flop reveal is handled with `animation-delay`.
const FADE_IN_KEYFRAMES = '__poker_card_fade_in';
const FADE_IN_CSS = `@keyframes ${FADE_IN_KEYFRAMES} { from { opacity: 0; } to { opacity: 1; } }`;
...
export function CardDisplay({
  ...
  // Plain CSS fade-in. Runs once on mount via the keyframes rule injected
  // below. `animation-fill-mode: backwards` keeps the card invisible during
  // the stagger delay so the staggered flop reveal still reads right. When
  // `suppressEntryMotion` is true we skip the animation entirely (used for
  // showdown staging where cards must not replay their entry).
  const animationStyle: CSSProperties = suppressEntryMotion
    ? {}
    : {
        animation: `${FADE_IN_KEYFRAMES} 220ms ease-out both`,
        animationDelay: `${dealDelay}s`,
      };

  return (
    <div className={`poker-card-wrapper ${className}`}>
      {/* Inject keyframes once per CardDisplay. Cheap — same rule each
          time; the browser dedupes by content. Avoids needing a global
          stylesheet edit for this one animation. */}
      <style>{FADE_IN_CSS}</style>
      {isFaceDown && (
        <div
          className="relative overflow-hidden"
          style={{ ...sizeStyle, boxShadow: cardShadow, ...animationStyle }}
        >
