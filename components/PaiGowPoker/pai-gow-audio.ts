/**
 * pai-gow-audio.ts — procedural sounds for /pai-gow-poker (Web Audio, no files).
 * A soft tick as each card deals, a flip on card pick / dealer reveal, a rising
 * major triad on a win, a felt thud on a loss, and a neutral tone on a push.
 * Ports the lab's tone()/sDeal/sFlip/sPick/sWin/sLose/sPush; same synth
...
class PaiGowAudio {
  ...
  /** Soft click as a card is tapped into the low hand. */
  playPick() {
    this.tone(520, 'triangle', 0.05, 0.14);
  }
...
export const paiGowAudio = new PaiGowAudio();
