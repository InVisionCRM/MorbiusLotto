...
/**
 * PaiGowInfoTabs — arcade2 info tabs for /pai-gow-poker:
 ...
 *   Odds        — settlement paytable + house rules
 *   FAQ         — provably-fair + game-specific answers
 *
 * Mirrors ThreeCardInfoTabs; "My rounds" + "FAQ" are the lab's two tabs, with
 * the public Recent/Leaderboard and the Odds tab added the way TCP does.
 */

import { useEffect, useState, type ReactNode } from 'react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { PaiGowHistory } from './PaiGowHistory';
import { ArcadeFAQ } from '@/components/arcade2/ArcadeFAQ';
import { ArcadeOddsTab } from '@/components/arcade2/ArcadeOddsTab';
import { paiGowFaqs } from './paiGowFaqs';
import { paiGowOdds } from './paiGowOdds';
import {
  fetchPaiGowRecent,
  fetchPaiGowLeaderboard,
  resultLabel,
  type PaiGowRecentHand,
  type PaiGowLeaderboardEntry,
  type PaiGowHistoryRound,
} from '@/lib/pai-gow-poker-client';

interface PaiGowInfoTabsProps {
  history: PaiGowHistoryRound[];
...
export function PaiGowInfoTabs({ history, historyLoading, onVerify }: PaiGowInfoTabsProps) {
  const [recent, setRecent] = useState<PaiGowRecentHand[]>([]);
  const [recentLoading, setRecentLoading] = useState(true);
  const [leaders, setLeaders] = useState<PaiGowLeaderboardEntry[]>([]);
  const [leadersLoading, setLeadersLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPaiGowRecent(25)
      .then((rows) => {
  ...
  return (
    <section aria-label="Pai Gow Poker information" className="arc-panel rounded-xl p-3 sm:p-4">
      <Tabs defaultValue="mine">
        ...
        <TabsContent value="mine" className="mt-3 focus-visible:outline-none">
          <PaiGowHistory rounds={history} loading={historyLoading} onVerify={onVerify} />
        </TabsContent>

        <TabsContent value="odds" className="mt-3 focus-visible:outline-none">
          <ArcadeOddsTab odds={paiGowOdds} />
        </TabsContent>

        <TabsContent value="faq" className="mt-2 focus-visible:outline-none">
          <ArcadeFAQ items={paiGowFaqs} accent="#22D3EE" />
        </TabsContent>
      </Tabs>
    </section>
  );
