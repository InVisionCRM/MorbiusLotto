...
const BallResult: React.FC<BallResultProps> = ({ number, type, animate, delay = 0 }) => {
  ...
  return (
    <div
      className={`relative inline-flex overflow-hidden rounded-full w-8 h-8 p-[1px] transition-all duration-500 ${motionClass}`}
    >
      {/* Ball base */}
      <span className="relative inline-flex items-center justify-center rounded-full font-bold w-full h-full text-md z-[1] bg-white text-purple-950 overflow-hidden">
