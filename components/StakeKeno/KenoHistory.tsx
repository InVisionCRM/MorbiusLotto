...
export function KenoHistory({ rounds, loading, onVerify }: KenoHistoryProps) {
  return (
    <section aria-label="Recent rounds" className="keno-panel rounded-xl p-3 sm:p-4">
      <div className="mb-2 flex items-baseline justify-between">
        <h2 className="keno-display text-sm font-semibold uppercase tracking-wider text-slate-300">
          Recent rounds
        </h2>
        ...
        <ul className="divide-y divide-cyan-950/60">
          {rounds.map((r) => {
            ...
            return (
              ...
              >
                <span className="keno-mono w-12 shrink-0 tabular-nums text-slate-500">
                ...
                <span className="keno-mono shrink-0 tabular-nums text-slate-400">
                ...
                <span
                  className={`keno-mono shrink-0 tabular-nums ${paid ? 'text-cyan-300' : 'text-slate-600'}`}
                >
                  {formatMultiplier(r.multiplierX100)}
                </span>
                <span
                  className={`keno-mono ml-auto shrink-0 tabular-nums font-semibold ${
