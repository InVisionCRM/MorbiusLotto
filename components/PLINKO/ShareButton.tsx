    ...
    try {
      ...
      statsDiv.innerHTML = `
        <div style="background: rgba(20,20,20,0.8); padding: 12px; border-radius: 8px; text-align: center; border: 1px inset rgba(60, 60, 60, 0.5);">
          <div style="color: rgba(34,211,238,0.8); font-size: 12px; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">Drops</div>
      ...
      chartClone.style.overflow = 'hidden'; // Hide any overflow
      ...
      if (svgElement) {
        ...
        svgElement.style.display = 'block'; // Remove any inline spacing
        svgElement.setAttribute('height', '460');
        svgElement.setAttribute('width', '760');
        svgElement.setAttribute('preserveAspectRatio', 'none');
      }

      // Remove any elements that might create empty space (like the history button overlay)
      const overlays = chartClone.querySelectorAll('[class*="absolute"]');
      overlays.forEach(overlay => {
        if (overlay.textContent?.includes('History')) {
      ...
      const canvas = await html2canvas(shareContainer, {
        ...
        imageTimeout: 0,
        onclone: (clonedDoc) => {
          // Ensure all images load properly in the cloned document
      ...
      // Draw text overlay directly on canvas (industry standard method)
      ...
      if (ctx) {
        ...
        const x = canvas.width / 2 / 2; // Center horizontally (divided by 2 because scale=2)
        const y = 550; // Position near bottom of chart area

        // Draw text shadow/background effect
      ...
      // Convert to blob and open modal
      canvas.toBlob((blob) => {
        if (blob) {
