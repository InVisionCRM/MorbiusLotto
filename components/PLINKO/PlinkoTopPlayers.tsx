...
import { usePlinkoTopPlayers, type PlinkoTopPlayerEntry } from '@/hooks/use-plinko-results'
...
export default function PlinkoTopPlayers() {
  const { data: players, isLoading, error } = usePlinkoTopPlayers(TOP_N)
  ...
  if (list.length === 0) {
    return (
      <p className="text-white/50 text-sm font-poppins text-center py-4">
        No player stats yet. Play Plinko to appear on the leaderboard.
      </p>
    )
  }
