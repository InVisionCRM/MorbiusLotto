...
/**
 * DiceFairnessModal — provably-fair panel for chips Dice.
 *
 * Same conventions as the keno/plinko/mines modals: set or randomize your
 * client seed (used for the next roll), and verify any past round by id.
 * Dice settles instantly, so the server seed is published with every round —
 * the committed-hash check (sha256(serverSeed) === serverSeedHash) is
 * recomputed locally with WebCrypto, and the multiplier is re-checked against
 * the published target/edge formula.
 */
...
export function DiceFairnessModal({
  open,
  onClose,
  clientSeed,
  onClientSeedChange,
  requestVerifyId,
}: DiceFairnessModalProps) {
  const [verifyId, setVerifyId] = useState('')
  const [result, setResult] = useState<DiceVerifyResult | null>(null)
  ...
  async function runVerify(id: string) {
    ...
    setHashMatches(null)
    try {
      const r = await verifyDice(trimmed)
      setResult(r)
            ...
            <section className="arc-panel space-y-3 rounded-lg p-3">
              <div className="space-y-1.5">
                ...
                <Check
                  ...
                  label="Multiplier matches the target/edge formula"
                />
              </div>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                <span>
                  Target: <span className="arc-mono text-slate-200">&lt; {formatX100(result.targetX100)}</span>
                </span>
                <span>
                  Roll:{' '}
